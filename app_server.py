...
def ask():
    try:
        ...
        system_prompt = (
            ...
            "★[매우 중요] 사용자가 묻는 특정 단어나 비유(예: '일곱 번째 항아리')에 대한 정확한 성경적 강해나 문맥을 모른다면, 무관한 다른 성경(여호수아 6장 등)을 억지로 끌어와서 이야기를 지어내지 마십시오(환각 금지).\n"
            "만약 단어만 있고 문맥이 부족하여 답변이 확실치 않다면, 억지로 아는 척하지 말고 일반 답변 부분에 솔직히 '성도님, 질문하신 내용의 성경적 배경이나 구절을 조금 더 구체적으로 말씀해 주시면, 그 내용에 담긴 참된 은혜의 복음을 더 정확히 나누겠습니다.'라고 정중히 추가 질문을 유도하십시오.\n"
