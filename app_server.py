...
def bg_generate_devotional(task_id, data):
    try:
        ...
        image_count = int(data.get('imageCount', 0))
        image_style = data.get('imageStyle', '고전유화')

        STUDIO_TASKS[task_id]['step'] = f'Gemini 3.6 Flash AI가 십자가 원고 작성 중... ({target_char_count:,}자 분량)'

        # 장르별 독립 구조, 서사 방식 및 문체/어조 전면 차별화 세팅
        if "수필" in genre:
            genre_desc = "실존적 고백과 서정적인 묵상이 어우러진 신앙 수필집(Faith Essay)"
            tone_desc = "나직하고 고백적이며 서정적인 신앙 수필 경어체 어조 (모든 문장의 끝은 반드시 '~입니다.', '~습니다.' 로만 종결)"
            structure_desc = """[신앙 수필 독자적 구성 지침 - 필히 엄수]
- 획일적인 딱딱한 번호(1. [제목])를 절대 쓰지 말고, 시적이고 감성적인 수필 소제목(예: ■ 바람 끝에 찾아오는 고백, ■ 내 안의 자랑을 비워내는 시간 등) 4~5개로 자연스럽게 단락을 전개하라.
- 서사 흐름:
  1) 일상의 한 장면이나 삶에서 마주하는 연약한 실존 고백
  2) 내면의 자기중심성과 거짓 열심에 대한 솔직한 성찰
  3) 그 연약함 위로 찾아오시는 십자가 예수 그리스도의 보혈과 은혜
  4) 내 힘을 내려놓고 하나님의 완전한 주권 안에 쉬는 안식
  5) 잔잔한 감사의 마침 기도문 ("예수 그리스도의 이름으로 기도드립니다. 아멘" 종결)"""

        elif "칼럼" in genre:
            genre_desc = "현대 기독교의 미혹을 비판하고 복음의 본질을 일깨우는 지성적 신학 칼럼(Theological Column)"
            tone_desc = "날카롭고 논리적이며 지성적인 시사/신학 칼럼 경어체 어조 (모든 문장의 끝은 반드시 '~입니다.', '~습니다.' 로만 종결)"
            structure_desc = """[신학 칼럼 독자적 구성 지침 - 필히 엄수]
- 언론 기고문/신학 논평 스타일의 명확한 소제목(예: ◆ 시대의 미혹: 성취 중심 종교성의 실체, ◆ 율법의 참된 기능과 인간 무능력의 깊이 등) 4~5개로 논리적으로 전개하라.
- 서사 흐름:
  1) 현대 교회와 성도들이 매몰되어 있는 인본주의적 쟁점 및 문제 제기
  2) 율법주의와 인간 중심 종교성의 허구에 대한 냉철한 성경적·신학적 비판
  3) 구속사적 관점에서 십자가 예수 그리스도의 참된 진리 선언
  4) 이 시대를 향한 신학적 제언과 은혜로의 권면
  5) 시대의 미혹을 이기는 마침 기도문 ("예수 그리스도의 이름으로 기도드립니다. 아멘" 종결)"""

        elif "자유" in genre:
            genre_desc = "깊이 있는 신학적 묵상과 영적 성찰을 유려하게 담아내는 자유 묵상 글(Free Theme Meditation)"
            tone_desc = "설교 형식을 완전히 배제하고 주제를 유려하고 정갈하게 풀어내는 묵상 경어체 어조 (모든 문장의 끝은 반드시 '~입니다.', '~습니다.' 로만 종결)"
            structure_desc = """[자유 주제 독자적 구성 지침 - 필히 엄수]
- 획일적인 번호나 고정된 소제목 틀에 매이지 말고, 선택한 자유 주제에 가장 잘 어울리는 유려한 소제목과 자유로운 단락 호흡으로 작성하라.
- 서사 흐름:
  1) 선택한 주제가 던지는 깊은 영적 질문과 신학적 화두 제시 (절대로 '성도 여러분 안녕하십니까' 등 설교식 인삿말 사용 금지!)
  2) 질문을 파고드는 영적 성찰과 인간 실존의 무능력 조명
  3) 십자가 보혈의 구속사적 비밀과 대속의 은혜 해방 선언
  4) 주권적 은혜 안에서 누리는 신앙의 자유와 평안
  5) 깊은 안식의 마침 기도문 ("예수 그리스도의 이름으로 기도드립니다. 아멘" 종결)"""

        else:  # 성경강해 (기본)
            genre_desc = "성경 본문의 구속사적 의미를 심층 풀이하는 정통 성경 강해(Bible Exegesis)"
            tone_desc = "깊이 있고 경건한 강해설교 어조 ('성도 여러분', '안녕하십니까', '~입니다', '~하십니까?')"
            structure_desc = """[성경 강해 독자적 구성 지침 - 엄수]
- 강해설교 구조 (1. [서론], 2. [본문 분석 및 구속사 강해], 3. [인본주의 종교성 폭로], 4. [십자가 복음 핵심 해설], 5. [성도의 삶 적용], 6. [결론 및 기도])에 따라 성경 구속사와 원어를 깊이 있게 풀이하라."""

        system_instruction = f"""당신은 故 김성수 목사님(서머나교회)의 신학 체계를 바탕으로 {genre_desc} 원고를 저작하는 전문 신학 작가입니다.

[장르별 차별화 지침 - 엄수]
- 장르: {genre} ({category})
- 선택된 장르({genre})의 고유한 서사 방식과 문체, 소제목 스타일을 철저히 반영하여 다른 장르와 명확히 차별화된 글로 작성하라.
- 어조 및 스타일: {tone_desc}
- 인간의 전적 파산과 무능력, 율법주의/기복주의 파쇄, 오직 십자가 예수 그리스도의 은혜만을 강조하라.
- 마무리는 간절한 기도문으로 종결하라 (반드시 "예수 그리스도의 이름으로 기도드립니다. 아멘" 로 끝맺음).

[경어체(~입니다, ~습니다) 문체 어미 필히 엄수 규정 - 매우 중요]
- '신앙수필', '자유주제', '신학칼럼' (및 성경강해 등 전체 장르) 원고의 본문 문장 종결 어미는 반말이나 독백체/해라체(~다, ~라)를 절대 쓰지 말라!
- 반드시 모든 문장의 끝을 정갈하고 격식 있는 경어체('~입니다.', '~습니다.')로만 끝맺어라!
  예시: "십자가의 복음은 인간의 자아를 완전히 파쇄합니다." / "우리는 오직 하나님의 주권과 은혜만을 인정해야 합니다."

[설교 형식 및 구어체 인삿말 엄격 금지 규정 - 매우 중요]
- '자유주제', '신앙수필', '신학칼럼' 원고에서는 설교 형식을 완전히 배제하라!
- 절대로 "성도 여러분 안녕하십니까", "사랑하는 성도 여러분", "청중 여러분", "오늘 말씀은..." 등의 설교용 인삿말이나 구어체 강단 설교조 호칭을 사용하여 시작하거나 작성하지 말라!
- 오직 선택된 장르({genre}) 본연의 정갈하고 품격 있는 경어체 어조로 작성하라.
...
※ 다시 한번 강조:
1. 선택하신 {genre} 장르 고유의 독자적인 구조와 서사 방식(수필은 감성 소제목, 칼럼은 논리 논평 소제목, 자유주제는 유려한 묵상 호흡)으로 차별화하여 작성하라.
2. 신앙수필, 자유주제, 신학칼럼의 문체는 반말(~다, ~라)을 절대 쓰지 말고, 반드시 문장 끝을 '~입니다.', '~습니다.'의 정갈한 경어체로 작성하라!
3. 자유주제/수필/칼럼 등 설교가 아닌 글에서는 "성도 여러분 안녕하십니까" 같은 설교 인삿말을 절대 사용하지 말고 설교 형식을 배제하라.
4. 원어는 과도하지 않게 5~10% 수준으로 절제하여 (한글 발음 - 한글 의미)를 표시하라."""
        ...
        if image_count > 0:
            ...
            STUDIO_TASKS[task_id]['step'] = 'AI 묵상 원고 최종 완성 정리 중...'
