...
def bg_generate_devotional(task_id, data):
    try:
        ...
        if "수필" in genre:
            ...
            tone_desc = "나직하고 고백적이며 깊은 울림을 주는 신앙 수필 경어체 어조 (모든 문장의 끝은 반드시 '~입니다.', '~습니다.' 로만 종결)"
            structure_desc = """구조:
...
5. [마음의 고백과 기도]: 따뜻하고 잔잔한 감사의 1인칭 묵상 기도문 ("예수 그리스도의 이름으로 기도드립니다. 아멘" 종결)"""
            ...
            tone_desc = "날카롭고 논리적이며 정갈한 시사/신학 칼럼 경어체 어조 (모든 문장의 끝은 반드시 '~입니다.', '~습니다.' 로만 종결)"
            structure_desc = """구조:
...
5. [매듭과 다짐]: 시대의 미혹을 이기는 결단과 마침 기도문 ("예수 그리스도의 이름으로 기도드립니다. 아멘" 종결)"""
            ...
            genre_desc = "깊이 있는 신학적 묵상과 영적 성찰을 담은 정갈한 묵상 에세이(Free Theme Meditation)"
            tone_desc = "설교 형식을 완전히 배제하고 깊은 영적 묵상과 성찰을 전하는 정중한 경어체 어조 (모든 문장의 끝은 반드시 '~입니다.', '~습니다.' 로만 종결)"
            structure_desc = """구조:
1. [묵상의 서두]: 주제가 던지는 깊은 영적 질문과 신학적 서두 (절대로 '성도 여러분 안녕하십니까' 등 설교식 인삿말 사용 금지!)
2. [인간 실존과 타락]: 인간의 전적 타락과 무능력에 대한 냉철한 진단
3. [십자가 복음의 비밀]: 구속사적 관점에서의 십자가 은혜와 해방 선언
4. [안식과 은혜의 삶]: 오직 예수 그리스도의 보혈 안에서 누리는 평안
5. [마침 기도]: 묵상을 마무리하는 1인칭 기도문 ("예수 그리스도의 이름으로 기도드립니다. 아멘" 종결)"""
...
- '신앙수필', '자유주제', '신학칼럼' (및 성경강해 등 전 전체 장르) 원고의 본문 문장 종결 어미는 반말이나 독백체/해라체(~다, ~라)를 절대 쓰지 말라!
...
- 오직 깊이 있는 신학 묵상글, 에세이, 칼럼 본연의 정갈하고 품격 있는 경어체 어조로 작성하라.
...
※ 다시 한번 강조:
1. 선택하신 {genre} 장르에 꼭 맞는 독자적인 구조와 어조로 작성하라.
