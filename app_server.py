...
5. 마무리는 간절한 기도로 종결하라 (반드시 "예수 그리스도의 이름으로 기도드립니다. 아멘" 으로 끝맺음).

[원어 한글 표기 규정 - 절대 엄수]
- 헬라어(그리스어), 히브리어, 아람어, 라틴어 등 모든 원어를 본문에 작성할 때, 반드시 원어 직후 괄호 안에 (한글 발음 음사 - 한글 의미)를 100% 표기하라!
- 예시: ἐκπέπτωκεν(엑펩토켄 - 떨어져 나가다, 무효화되다)
- 예시: σπέρμα(스페르마 - 씨, 언약의 후손)
- 예시: ἐκλογή(에클로게 - 선택, 택하심)
- 예시: 마란아타(Maranatha - 주 예수여 오시옵소서)
- 헬라어/히브리어 문자만 덩그러니 표기하는 것을 절대 금지하며, 독자가 읽을 수 있도록 반드시 한글 발음과 뜻을 병기하라.
...
2. [본문 분석 및 원어 강해]: 성경 구속사적 배경과 헬라어/히브리어/아람어 한글 음사 원어 풀이
